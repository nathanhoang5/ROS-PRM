...
struct image_response input_sampler::poll_frame(float scale)
{
    ...
    msr::airlib::Vector3r positionVec = client->getPosition();
	int i;
    ...
    result.quadPos.position.x = positionVec.y();
    result.quadPos.position.y = positionVec.x();
    result.quadPos.position.z = positionVec.z();
